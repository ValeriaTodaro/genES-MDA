...
def localization(ens_par,par,obs,loc_space,loc_time,iter_loc):
    a_space=2500   #correlation length in space
    a_time=18000    #correlation length in time
    time_par=par[:,3]
    pos_obs=obs[:,0:3]
    time_obs=obs[:,3]
    if iter_loc=='n':
