...
def Func_ens(par, ens):
    ...
    (MinT,MaxT)=(1e-3,1e-5)
    (MinS,MaxS)=(1e-5,1e-6)
    Ensemble[0,:]=EnsembleGenerator.ConstantRandom(MinT,MaxT,1,ens)
    Ensemble[1,:]=EnsembleGenerator.ConstantRandom(MinS,MaxS,1,ens)
    return Ensemble
...
def localization(ens_par,par,obs,loc_space,loc_time,iter_loc):
    a_space=70       #correlation length in space
...
def Metrics_obs_par(Xprev,pred,True_par,obs):
    ...
    metrics_dict ={}
    for variable in ['RMSE_obs', 'RMSE_par']:
        metrics_dict[variable]=eval(variable)
    return metrics_dict